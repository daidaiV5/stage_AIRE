...
if("--help" %in% Args) {
  cat("The R Script gcnScript.r
      ...
      Example:
      gcnScript.r --in_mat=EXAMPLE_raw_count_sub.csv  --out=output --dir=OUTDIR \n\n")
...
##### LOAD DATA #######################################################
vst2 <- as.matrix(read.csv(file_mat,
